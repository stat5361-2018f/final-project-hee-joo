...
&=exp(\sum_{i=1}^{n}(-\frac{1}{2}tr[\Sigma(X^i;\phi^{\ast})^{-1}S(Y^i,X^i;\beta))-\frac{1}{2}log|D(\phi)| +\frac{m}{2} log 2\pi)))) 
\end{align*}
...
$$U^*_2(\boldsymbol{\beta}, \boldsymbol{\phi}) = -\sum^n_{i=1} \boldsymbol{W(X)}^\prime_i[G()], \quad  \boldsymbol{I}^*_{22} = n W$$
where $W = \sum^n_{i=1} \sigma^{-2}_i W_t$, $W_t = \sum^{t-1}_{j=1} \sum^{t-1}_{k=1} \sigma_{jk} z_{tk} z^\prime_{tl}$, and $\boldsymbol{r}_i = \boldsymbol{y}_i - \boldsymbol{X}_i \boldsymbol{\beta} = (r_{1i}, \ldots, r_{mi})^\prime$. $\boldsymbol{Z}(i) =(z(i,1), \ldots, z(i,m))^\prime$ is the $m \times q$ matrix where $z(i,t) = \sum^{t-1}_{j=1}r_{ij}z_{tj}$. Given $\sigma^2_i$, $(\hat{\boldsymbol{\beta}}, \hat{\boldsymbol{\phi}})$ are obtained using Fisher scoring algorithm.
...
When you look at the tables, the average cumulative coefficient estimates look like very close to the true value. When we focus on the estimates corresponding the covariate, the standard errors of estimates for mean are very similar. The standard errors of estimates for variance are a little different. Standar error for $(\phi_{2,1}$ is higher than for $\phi_{1,2}$ and $\phi_{3,2}$.
